...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Segment point1 = {{-0.45f, 0.4f, 0.0f} , {1.0f, 0.5f, 0.0f}};
	...
	while (Novice::ProcessMessage() == 0) {
		...
		ImGui::DragFloat3("Point2Normal" , &point2.normal.x , 0.01f);
		point2.normal = Normalize(point2.normal);
		ImGui::DragFloat("Point2Distance" , &point2.distance , 0.01f);

		if (IsSegmentToPlaneCollision(point1 , point2)) {
			color = RED;
		} else {
			color = WHITE;
		}
